...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Enhanced dashboard data using new API
    final dashboardParams = {
      'month': _selectedMonth?.month,
      'year': _selectedMonth?.year,
    };
    final dashboardAsync =
        ref.watch(dashboardOverviewProvider(dashboardParams));
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: RefreshIndicator(
          onRefresh: () async {
            _refreshData();
            // Also refresh dashboard-specific providers
  ...
  Widget _buildSummaryCards(
    ...
    // Use enhanced dashboard data if available, otherwise calculate manually
    double totalSales = 0.0;
    double totalExpenses = 0.0;
    double totalCommission = 0.0;
    int totalProducts = 0;

    // Try to use enhanced dashboard data first
    ...
    // Fallback to manual calculation if enhanced data is not available
    if (dashboardAsync.hasValue == false || dashboardAsync.value == null) {
